...
<<Promise.all, `Promise.all`>> 接收一个 promise对象的数组作为参数，当这个数组里的所有promise对象全部变为resolve状态的时候，它才会去调用 `.then` 方法。
...
<<Promise.all, `Promise.all`>> 在以下几点和之前的例子不太一样。
...
`timerPromisefy` 会每隔一定时间（通过参数指定）之后，都会返回一个promise对象，状态为FulFilled，其状态值为传给 `timerPromisefy` 的参数。
...
也就是说，这个promise对象数组中所有promise都变为resolve状态的话，至少需要128ms。实际我们计算一下<<Promise.all, `Promise.all`>> 的执行时间的话，它确实是需要128ms。

从上述结果可以看出，传递给 <<Promise.all, `Promise.all`>> 的promise并不是一个个的顺序执行的，而是并行执行的。
...
要想了解更多关于如何使用Promise进行串行处理的内容，可以参考第4章的<<promise-sequence, Promise中的串行处理>>。
====