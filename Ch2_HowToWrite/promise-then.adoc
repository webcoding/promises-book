...
这其中，我想大家已经认识了 `.then().catch()` 这种方法链式的写法了，其实在Promise里可以将任意个方法连在一起作为一个方法链（method chain）。
...
如果把在 `then` 中注册的每个回调函数称为task的话，那么我们就可以通过Promise方法链方式来编写taskA -> task B 这种流程的逻辑了。

Promise方法链这种叫法有点长（其实是在日语里有点长，中文还可以 --译者注），因此后面我们会简化为 <<promise-chain,promise chain>> 这种叫法。

Promise之所以适合编写异步处理较多的应用，promise chain 应该算是其中的一个原因吧。

在本小节，我们将主要针对使用 `then` 的promise chain的行为和流程进行学习。

=== promise chain

在第一章 <<promise-chain,promise chain>> 里我们看到了一各很简单的 
...
* 发生异常的时候
* 返回了一个Rejected状态的promise

在 <<how-to-write-promise,第一章>> 中我们已经看到，Promise中的处理习惯上都会采用 `try-catch` 的风格，当发生异常的时候，会被 `catch` 捕获并被由此函数注册的回调函数进行错误处理。
...
[NOTE]
====
在本例中我们在taskA中使用了 `throw` 方法故意制造了一个异常。但在实际中想主动进行onRejected调用的时候，应该返回一个Rejected状态的promise对象。关于这种两种方法的异同，请参考 <<not-throw-use-reject, 使用reject而不是throw>> で解説しています。
====
...
[NOTE]
关于这部分内容可以参考 <<then-return-new-promise,每次调用then都会返回一个新创建的promise对象>> ，那里也对一些常见错误进行了介绍。
