...
那么让我们先来学习下如何使用http://visionmedia.github.io/mocha/[Mocha]来对Promise 进行基本的测试吧。
...
Mocha是Node.js下的测试框架工具,在这里，我们并不打算对http://visionmedia.github.io/mocha/[Mocha]本身进行详细讲解。对
http://visionmedia.github.io/mocha/[Mocha]感兴趣的读者可以自行学习。
...
最后至于为什么说_支持"Promise测试"_，这个我们在后面再讲。
...
首先，让我们试着编写一个对传统回调风格的函数进行测试的代码。

=== 回调函数风格的测试

在<<promise-is-always-async,栏: Promise只能进行异步操作？>>中我们已经讲过，
Promise中对 `then` 绑定的函数的调用总是异步进行的。
