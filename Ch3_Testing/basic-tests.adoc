...
----
<1> 创建名为`Fulfilled` 的promise object
<2> 调用`done` 标明测试结束


<<Promise.resolve, `Promise.resolve`>> 用来返回promise object，
返回的promise object状态为FulFilled。
结果，调用绑定到`.then` 的回调函数。

<<promise-is-always-async,column: Promise总是异步?>> 中也提到的那样，
promise object总是异步处理的，所有测试同样需要用异步对应的方式编写。
